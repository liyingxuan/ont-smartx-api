...
class TokenProxy
{
     ...
     * @return $this|\Illuminate\Http\JsonResponse
     */
    public function githubLogin($github_id, $github_password, $github_name)
    {
        $UserInfo = User::where('github_id', $github_id)->first();
        if($UserInfo->name){
            $github_password = $UserInfo->password;
        }

        if (auth()->attempt(['github_id' => $github_id, 'password' => $github_password])) {
            $user = auth()->user();


            $UserInfo->github_name = $github_name;
            $UserInfo->save();
